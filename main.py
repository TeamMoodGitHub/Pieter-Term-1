...
def basics():
	...
	with open('data.pickle', 'wb') as f:
		...
		pickle.dump(regionID, f, pickle.HIGHEST_PROTOCOL)
		pickle.dump(accountID, f, pickle.HIGHEST_PROTOCOL)
		pickle.dump(ID, f, pickle.HIGHEST_PROTOCOL)
		pickle.dump(gameID, f, pickle.HIGHEST_PROTOCOL)
		pickle.dump(champion, f, pickle.HIGHEST_PROTOCOL)
...
def details(game):
	...
	if len(deaths) != 0:
		...
		for i in range(0,len(deaths)):
			if i == 0:
				plt.axvline(deaths[i], ymin=0, ymax=1, color='tomato', label='Deaths')
			else:
				plt.axvline(deaths[i], ymin=0, ymax=1, color='tomato')
	...
	return render_template("details.html", plot_url=plot_url, plot_url2=plot_url2)
